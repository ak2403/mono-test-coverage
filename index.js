...
function writeCoverageReportToFile(coverageReport) {
  ...
  fs.writeFileSync(`coverage/coverage-total.${createDateTimeSuffix()}.json`, JSON.stringify(coverageReport, null, 2));
}
...
try {
  ...
  const currCoverageReport = readSummaryPerPackageAndCreateJoinedSummaryReportWithTotal(packagesSummaryPaths);
  // 2. Calculate diff
  ...
  core.setOutput('coverage', JSON.stringify(coverageReportForVisualRepresentation));
} catch (error) {
